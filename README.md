...
```yaml
semester: 115-1         # 民國學年度-學期,例如 115-1(115學年度上學期)、115-2(115學年度下學期),選填
...
- **Scratch 作業自動批改**:不在這個教材網站範圍內,但已經有獨立專案在做這件事——`Scratchy`(Next.js + Prisma + scratch-vm,部署在Render),學生上傳 `.sb3` 後自動跑模擬比對參考解答＋靜態規則檢查、算出建議分數,老師審核後公布。這個網站的講義只負責連過去(見 `week4-8-intro.md` 的繳交連結),不嵌入、不重做評分邏輯。
